...
#MANEXSTRT
fsm_write_binary_file = foma.fsm_write_binary_file
fsm_write_binary_file.restype = c_int
#MANEXEND
...
class FST(object):
    ...
    def load(cls, filename):
        ...
        fsm = cls()
#        fsm.fsthandle = foma_fsm_read_binary_file(c_char_p(filename.encode("utf8")))
#        print ("Load <"+filename+">")
        fsm.fsthandle = foma_fsm_read_binary_file(filename.encode("utf-8"))
    ...
    def save(self, filename):
        """Load binary FSM from file."""
        if self.fsthandle:
          fsm_write_binary_file(self.fsthandle, c_char_p(filename))
        else:
            raise ValueError("There is no FSM defined.")
    #MANEXEND
...
class MTFSM(FST):
    ...
    def __init__(self, regex = False, numtapes = 2):
        if isinstance(regex, bytes) or isinstance(regex, str):
...
#FSM = FST()
#FS = FSM.load("/Users/jbt694/project_poetryevaluation/erato/models/eu/poetic_features/silabaEus.fst")
#print (list(FS.apply_down("etxerakoan")))
